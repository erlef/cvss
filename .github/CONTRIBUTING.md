...
- [Report a bug](https://github.com/erlef/cvss/issues/new?labels=bug&template=BUG.md)
- [Propose a new feature](https://github.com/erlef/cvss/issues/new?labels=enhancement&template=FEATURE.md)
- [Send a pull request](https://github.com/erlef/cvss/pulls)
...
<!--
Please do not report a bug for a version of `cvss` that is no longer
supported (`< 1.0.0`). -->
Please do not report a bug if you are using a version of Erlang that is not
supported by the version of `cvss` you are using.
...
We are trying to keep backwards compatibility breaks in `cvss` to a
...
## Using `cvss` from a Git checkout

The following commands can be used to perform the initial checkout of
`cvss`:

```bash
$ git clone git@github.com:erlef/cvss.git

$ cd cvss
```

Install `cvss`'s dependencies using [rebar3](https://rebar3.org/):

```bash
$ rebar3 get-deps
```

## Running `cvss`'s test suite

After following the steps shown above, `cvss`'s test suite is run like
this:
